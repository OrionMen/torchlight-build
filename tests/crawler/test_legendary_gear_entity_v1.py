...
class LegendaryGearEntityV1Test(unittest.TestCase):
    ...
    def test_exactly_332_legendary_entities_after_refetch(self):
        self.assertEqual(332, self.report["total_entities"])
        self.assertEqual(7, self.report["excluded_non_equipment"])
        self.assertEqual(0, self.report["skipped_empty"])
        self.assertEqual(332, self.report["corrosion_enriched"])
    ...
    def test_non_equipment_is_absent_and_refetched_snapshot_is_present(self):
        self.assertNotIn("tlidb:cn:Sparks_of_Moth_Fire", self.by_id)
        self.assertIn("tlidb:cn:Frozen_Flame", self.by_id)
